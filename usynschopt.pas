...
begin
 //SelectTheme(cbThemes.ItemIndex, OldSyntaxSchemeList);
 ...
 clBLineBackSelect(Sender);
 clBLineForeSelect(Sender);
 clBMainForeSelect(Sender);
 clBMainBackSelect(Sender);
 *)
end;
...
begin
 ...
 For I := 0 To Main.PageDock.PageCount - 1 Do
 //For I := Main.PageDock.PageCount - 1 DownTo 0 Do
  If (Main.PageDock.Pages[I] Is TTabForm) Then
   If (Main.PageDock.Pages[I] As TTabForm).ParentForm Is TEditor Then
    Begin
     Tmp := (Main.PageDock.Pages[I] As TTabForm).ParentForm;
  ...
  Begin
   For I := 0 To WorkingSynSch.AttrCount - 1 Do
    Begin
     ...
     If WorkingSynSch.Attribute[I].StoredName = SYNS_AttrFunction Then
      WorkingSynSch.Attribute[I].Foreground := PiNoteThemes[ThemeIdx].MainForeground;

     If WorkingSynSch.Attribute[I].StoredName = SYNS_AttrDataType Then
      WorkingSynSch.Attribute[I].Foreground := PiNoteThemes[ThemeIdx].MainForeground;

     If WorkingSynSch.Attribute[I].StoredName = SYNS_AttrPragma Then
      WorkingSynSch.Attribute[I].Foreground := PiNoteThemes[ThemeIdx].ColorPreProcessor;
   end;
