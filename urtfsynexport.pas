...
begin
 If BackgroundChanged Then
  Begin
   AddData(Format('\cb%d', [GetColorIndex(fLastBG) + 1]));
 ...
 If ForegroundChanged Then
  Begin
   AddData(Format('\cf%d', [GetColorIndex(fLastFG) + 1]));
...
begin
 ...
 Result := Result + Format('\deflang1033\pard\plain\highlight0\cb1\f0\fs%d ', [2 * Font.Size]);
end;
