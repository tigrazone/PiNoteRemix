...
uses
  ...
  SynHighlighterIni, umysynhighlighterpo,
  ...
  //MySynHighlighterTclTk, MySynHighlighterInno, MySynHighlighterHC11,
  Graphics,
  ...
  MySEHighlighterV, MySEHighlighterMatlab, MySEHighlighterSwift,
...
Procedure CreateSyntaxSubMenu(ParentMenu : TMenuItem);
...
begin
 ...
 //18/03/22
 SyntaxSchemeList.Add(TMySEHighlighterSwift.Create(Nil));          //58- Swift
 OldSyntaxSchemeList.Add(TMySEHighlighterSwift.Create(Nil));
 SyntaxSchemeList.Add(TMySEHighlighterDart.Create(Nil));          //59- Dart
 OldSyntaxSchemeList.Add(TMySEHighlighterDart.Create(Nil));

 //16/01/23
 SyntaxSchemeList.Add(TMySEHighlighterAutoIt.Create(Nil));        //60- AutoIt
 OldSyntaxSchemeList.Add(TMySEHighlighterAutoIt.Create(Nil));
 SyntaxSchemeList.Add(TMySEHighlighterErlang.Create(Nil));        //61- Erlang
 OldSyntaxSchemeList.Add(TMySEHighlighterErlang.Create(Nil));
 SyntaxSchemeList.Add(TMySEHighlighterProtoBuf.Create(Nil));      //62- ProtoBuf
 OldSyntaxSchemeList.Add(TMySEHighlighterProtoBuf.Create(Nil));
 SyntaxSchemeList.Add(TMySEHighlighterKotlin.Create(Nil));        //63- Kotlin
 OldSyntaxSchemeList.Add(TMySEHighlighterKotlin.Create(Nil));
...
procedure CreateSyntaxSubMenu(ParentMenu: TMenuItem);
 Var NewItem, SubItem : TMenuItem;
     Ind, IndD, NumD : Integer;
begin
 If Assigned(ParentMenu) Then
  Begin
   ParentMenu.Clear;

   NewItem := TMenuItem.Create(ParentMenu);
   NewItem.Caption := 'Default Text';
   NewItem.Tag := -1;
   NewItem.OnClick := @Main.OnSubMenuSyntaxSchemeClick;

   ParentMenu.Add(NewItem);

   For IndD := 0 To SyntaxSchemeNameDiv.Count - 1 Do
    Begin
     NumD := GetNumberOfSchemeDiv(SyntaxSchemeNameDiv[IndD]);

     If NumD > 1 Then
      Begin
       NewItem := TMenuItem.Create(ParentMenu);
       NewItem.Caption := SyntaxSchemeNameDiv[IndD];
       NewItem.Tag := -2;

       ParentMenu.Add(NewItem);

       For Ind := 0 To SyntaxSchemeNameList.Count - 1 Do
        If SyntaxSchemeNameList[Ind][1] = SyntaxSchemeNameDiv[IndD] Then
         Begin
          SubItem := TMenuItem.Create(NewItem);
          SubItem.Caption := SyntaxSchemeNameList[Ind];
          SubItem.Tag := GetSyntaxSchemeIndex(SyntaxSchemeNameList[Ind]);
          SubItem.OnClick := @Main.OnSubMenuSyntaxSchemeClick;
      ...
      Begin
       For Ind := 0 To SyntaxSchemeNameList.Count - 1 Do
        If SyntaxSchemeNameList[Ind][1] = SyntaxSchemeNameDiv[IndD] Then
         Begin
          NewItem := TMenuItem.Create(ParentMenu);
          NewItem.Caption := SyntaxSchemeNameList[Ind];
          NewItem.Tag := GetSyntaxSchemeIndex(SyntaxSchemeNameList[Ind]);
          NewItem.OnClick := @Main.OnSubMenuSyntaxSchemeClick;
